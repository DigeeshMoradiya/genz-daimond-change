                ...
                <div key={index} className="group relative" onMouseEnter={() => Boolean(subMenuItems[item.href]) ? setActiveMegaMenu(item.href) : null} onMouseLeave={() => Boolean(subMenuItems[item.href]) ? setActiveMegaMenu(null) : null}>
                    ...
                    }
                    {
                        Boolean(subMenuItems[item.href]) ? (
                            <div className={`z-[1] fixed left-0 right-0 lg:group-hover:block shadow-lg px-4  transition-all duration-300 ease-in-out bg-white lg:max-h-[70vh] overflow-y-auto ${Boolean(item.href === isMegaMenu) ? 'lg:hidden top-[151px]  right-0 left-0 block' : 'hidden'}`} >
